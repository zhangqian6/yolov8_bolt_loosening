...
class BasePredictor:
    ...
    def setup(self, source=None, model=None, return_outputs=False): # 进行设置
        # source
        source = str(source if source is not None else self.args.source) # 图片或视频数据绝对路径
        is_file = Path(source).suffix[1:] in (IMG_FORMATS + VID_FORMATS) # 数据格式是否正确 bool类型
        is_url = source.lower().startswith(('rtsp://', 'rtmp://', 'http://', 'https://'))# 是否网上资源 bool
    ...
    def __call__(self, source=None, model=None, return_outputs=False):
        ...
        for batch in self.dataset:
            ...
            # 检查是否处于暂停状态
            while self.paused:
                print("Stream paused... waiting to resume.")
                cv2.waitKey(1000)
    ...
    def predict_cli(self, source=None, model=None, return_outputs=True):
        for model, tm, im0s in (self.__call__(source, model, return_outputs)):
            yield model, tm, im0s
            #pass
